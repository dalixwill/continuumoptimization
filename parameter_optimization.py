...
class OptimizationModel:
    ...
    def trainModel(self, X, Y):
        ...
        if self.kernelIndex == 2:
            self.kernel = 1.0*RBF(length_scale=np.ones(np.size(X, 1))) 
...
class Reference():
    ...
    def __init__(self, sourceDir='/home-1/dwill164@jhu.edu/projects/04--Parameter_Optimization/CuZr_Ref/',
        ...
        try:    
            ...
            if os.path.isfile(finalCMStrainField):
                if om.distanceMetric == 0:
                    ...
                    self.distance, self.r0, self.rR = compareMatrices(A, B, \
                        stol=om.rankTolerance, constantrank=om.decompositionRank, \
                        verbose=om.verboseDecomposition)
    ...
    if checkPrevious and os.path.isfile(prevFileName):
        ...
        if om.samplingstyle == 0:
            ...
            *aa, = np.meshgrid(*AA.T)
            ...
            distribution = cp.J(*A)
            X = distribution.sample(om.nInitialObs - 2**(initParams.ndims), \
                    rule='S', seed=nseed)
            X = X.T
            X = np.append(X, Xe, axis=0)
            ...
            # Random sampling
            X = np.zeros((optimizationModel.nInitialObs - 2**(initParams.ndims), initParams.ndims))
            Y = []
            ...
            for param in initParams.parameters:
                if not param.is_constant:
                    X[:, i] = np.random.uniform(param.vmin, param.vmax, \
                        optimizationModel.nInitialObs - 2**(initParams.ndims))
...
def plotComparison(parameters, optimizationModel, reference, X, Y):
    ...
    if reference.fromContinuum:
        ...
        plt.contourf(Tf_Ref, cmap='Greys')
        plt.colorbar()
        plt.axis('off')
        ...
        plt.contourf(Exy_Ref, cmap='Greys')
        plt.colorbar()
        plt.axis('off')
...
def compareMatrices(A, B, stol=1e-4, method=0, \
    ...
    # Reduce UA, UB to min rank:
    UA = UA[:, :r]
    UB = UB[:, :r]

    if method == 0:
        # Compute Grassmannian distance between UA, UB
        R = np.dot(UA.T, UB)
...
def computeContinuumDeviatoricStress(filePath, frameNum):
    ...
    return np.sqrt(s**2. + tau**2. + 3.*q**2.)
...
def main():
    ...
    om = OptimizationModel(nInitialObs=2000, nTotalObs=10000, nSurrEvals=50000, \
                            samplingstyle=1,\
    ...
    beta = Parameter('beta', 8.00 , vmin=2, vmax=18)
    u0 = Parameter('u0', -3.3626, vmin=-3.39, vmax=-3.36)
    chi_len = Parameter('chi_len', 15., vmin=1., vmax=20.)
    eps0 = Parameter('eps0', 0.3, vmin=0.1, vmax=0.5)
    c0 = Parameter('c0', 0.3, vmin=0.1, vmax=1.0)
    omega = Parameter('Omega', 370., vmin=300., vmax=500.)
    theta = Parameter('theta', 100., vmin=10., vmax=400.)
    delta = Parameter('Delta', 8000., vmin=7800., vmax=8800.)
    chi_inf = Parameter('chi_inf', 3000)
    sy = Parameter('sy', 0.85, vmin=0.5, vmax=1.5)
    mu = Parameter('mu', 20)
